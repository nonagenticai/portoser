    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                status = 500
                logger = MetricsContextLogger(__name__)
                logger.error(
                    f"Request failed: {endpoint}", method=method, endpoint=endpoint, error=str(e)
                )
                raise
            finally:
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            try:
                ...
                metrics_registry.cache_operations_total.labels(
...
def monitor_queue_operation(queue_name: str):
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                metrics_registry.queue_operations_total.labels(
                    queue_name=queue_name, operation=operation
                ).inc()

                return result
            finally:
                duration = time.time() - start_time
                metrics_registry.queue_processing_duration_seconds.labels(
...
def generate_grafana_dashboard() -> Dict:
    ...
    return {
        "dashboard": {
            "title": "Metrics System Monitoring",
            ...
            "panels": [
                ...
                },
                {
                    ...
                    "type": "stat",
                    "title": "Error Rate",
                    "targets": [{"expr": "error_rate", "legendFormat": "Error %"}],
                    "options": {"graphMode": "area", "colorMode": "value"},
                    "thresholds": {
                        "mode": "absolute",
                        "steps": [
                ...
                },
                {
                    ...
                    "targets": [{"expr": "cache_hit_rate", "legendFormat": "Hit %"}],
                    "options": {"graphMode": "area", "colorMode": "value"},
                    "thresholds": {
                        "mode": "absolute",
                        "steps": [
                ...
                },
                {
                    ...
                    "type": "stat",
                    "title": "Queue Depth",
                    "targets": [{"expr": "queue_depth", "legendFormat": "Items"}],
                    "options": {"graphMode": "area", "colorMode": "value"},
                    "thresholds": {
                        "mode": "absolute",
                        "steps": [
                ...
                },
                {
                    ...
                    "yaxes": [{"format": "reqps", "label": "Requests/sec"}, {"show": False}],
                },
                # Row 3: Errors and Queue
                {
                    ...
                    "yaxes": [{"format": "short", "label": "Errors/sec"}, {"show": False}],
                },
                {
                    "id": 8,
                ...
                # Row 4: Cache and Database
                {
                    ...
                    "targets": [
                        {
                            "expr": "sum(rate(cache_operations_total{result='hit'}[5m]))",
                            "legendFormat": "Hits",
                        },
                        {
                            "expr": "sum(rate(cache_operations_total{result='miss'}[5m]))",
                            "legendFormat": "Misses",
                        },
                    ],
                    "yaxes": [{"format": "ops", "label": "Operations/sec"}, {"show": False}],
                },
                {
                    "id": 10,
                ...
                # Row 5: Business Metrics
                {
                    ...
                    "targets": [
                        {
                            "expr": "sum(rate(metrics_ingested_total[5m])) by (source)",
                            "legendFormat": "{{source}}",
                        }
                    ],
                    "yaxes": [{"format": "short", "label": "Metrics/sec"}, {"show": False}],
                },
                {
                    "id": 12,
                    ...
                    "type": "graph",
                    "title": "Active Users",
                    "targets": [{"expr": "active_users", "legendFormat": "Active Users"}],
                    "yaxes": [{"format": "short", "label": "Users"}, {"show": False}],
                },
            ],
            "templating": {
                "list": [
...
def example_usage():
    ...
    @monitor_request("/api/data")
    def get_data(method="GET"):
        """Example API endpoint"""
        logger.info("Fetching data", endpoint="/api/data", user_id=123)
    ...
    def cache_get(key: str):
        ...
        return "value" if random.random() > 0.3 else None

    # Example queue function
    @monitor_queue_operation("processing")
    ...
    logger.info("Starting example monitoring")
    ...
    for i in range(10):
        ...
        metrics_collector.record_cache_access(result is not None)

        # Queue operations
        metrics_registry.queue_depth.labels(queue_name="processing").set(100 + i * 10)
        process_queue_item({"id": i})

    # Check current metrics
    current_metrics = metrics_collector.get_current_metrics()
    logger.info("Current metrics", **current_metrics)

    # Evaluate alerts
    alerts = alert_manager.evaluate_alerts(current_metrics)
    if alerts:
        logger.warning(f"Active alerts: {len(alerts)}", alerts=alerts)

    # Get metrics for Prometheus
    metrics_output, content_type, status = metrics_endpoint()
    ...
    print("\nGrafana Dashboard (panels count):", len(dashboard["dashboard"]["panels"]))
