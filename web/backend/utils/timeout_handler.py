...
async def example_usage():
    """Demonstrate usage of the timeout and retry mechanisms."""

    print("=" * 70)
    print("Timeout Handler and Retry Logic - Example Usage")
    print("=" * 70)

    # Example 1: Simple subprocess with timeout
    print("\n1. Simple subprocess with timeout:")
    result = await async_subprocess_with_timeout(["echo", "Hello, World!"], timeout=5.0)
    print(f"   Success: {result.success}")
    print(f"   Output: {result.stdout}")
    print(f"   Execution time: {result.execution_time:.3f}s")

    # Example 2: Command that will timeout
    print("\n2. Command with timeout (sleep 10s with 2s timeout):")
    result = await async_subprocess_with_timeout(["sleep", "10"], timeout=2.0)
    print(f"   Success: {result.success}")
    print(f"   Error category: {result.error_category.value if result.error_category else 'None'}")
    print(f"   Execution time: {result.execution_time:.3f}s")

    # Example 3: SSH command with retry
    print("\n3. SSH command with retry (will fail - example):")

    @retry_with_backoff(max_attempts=3, base_delay=1.0)
    async def test_ssh():
        return await ssh_execute_with_timeout("nonexistent.host", "uptime", timeout=2.0)

    result = await test_ssh()
    print(f"   Success: {result.success}")
    ...
    degraded = handle_graceful_degradation(result, "system_uptime")
    print(f"   Status: {degraded['status']}")
    print(f"   Error category: {degraded['error']['category']}")
    print(f"   Fallback mode: {degraded['fallback']}")
